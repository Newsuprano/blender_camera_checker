...
class MainWindow(QMainWindow) :
    def __init__(self) :
        ...
        # Restrict the toolbar strictly to Top or Bottom areas
        ...
        # 2. Use Existing Pipeline Action
        ...
        # Add the menu button directly to the toolbar (pushed right by the spacer)
        ...
        self.total_label = QLabel(f"<b>Total Frames:</b> {total_frames}")
    ...
    def update_toolbar_display_mode(self):
        ...
        # Reset bounds to safe defaults since it's restricted to top/bottom
    ...
    def on_run_new_pipeline(self):  
        ...
        if not self.blender_exe_path or not Path(self.blender_exe_path).exists():
            QMessageBox.critical(
                self, 
                "Invalid Executable", 
                f"The Blender executable path is invalid or does not exist:\n{self.blender_exe_path}\n\nPlease check your settings."
            )
            return
        ...
        if result_code == QDialog.DialogCode.Accepted:
            try:
                ...
                # Grab the count collected from the progress dialog
                extracted_count = progress_dialog.extracted_count
    ...
    def update_info_panel(self, df):
        ...
        self.total_label.setText(f"<b>Total Frames:</b> {total_frames}")
...
class PipelineConfigDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # Load from cache and pre-fill fields AFTER widgets are created
        cache = load_cache()
        saved_input = cache.get("last_input_dir", "")
        saved_output = cache.get("last_output_dir", "")

        if saved_input:
            self.input_input.setText(saved_input)
            self.input_dir = saved_input
            
        if saved_output:
            self.output_input.setText(saved_output)
            self.output_dir = saved_output
...
class ProgressDialog(QDialog):
    ...
    def on_finished(self, success, err_str, count):
        self.is_extracting = False
